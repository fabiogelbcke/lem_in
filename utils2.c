...
/*   Updated: 2016/11/14 16:49:16 by fschuber         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		add_connections(t_node **graph, char **map, char **names, int i)
{
	char	**nodes;
	int		j;
	int		k;
...
t_node		**read_graph(char **map)
{
	...
	is_start_finish = 0;
	j = alloc_graph(map, &graph);
	init_nodes(&graph, map, j);
	...
	names[j] = NULL;
	add_connections(graph, map, names, 0);
	return (graph);
}
